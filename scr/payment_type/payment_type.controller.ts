...
async function findAll( req: Request, res: Response){
    try{
        ...
        res.status(200).json({message: "Found all payment types", data: payment_types});
    }catch(error: any){
...
async function add(req: Request, res: Response){
    try{
        //TODO check if payment type exists and if payment type is valid 
        const payment_type = em.create(PaymentType, req.body);
        await em.flush();
        res.status(201).json({message: "Payment type created", data: payment_type});
    }catch(error: any){
        res.status(500).json({message: error.message});
    }
}
async function update(req: Request, res: Response){
    try{
        ...
        await em.flush();   
        res.status(201).json({message: "Payment type updated", data: payment_typeToUpdate});
    }catch(error: any){
        res.status(500).json({message: error.message});
    }
}
...
async function remove(req: Request, res: Response){
    try{
        ...
        res.status(201).json({message: "Payment type deleted", data: payment_type});
    }catch(error:any){
        res.status(500).json({message: error.message});
    }
}
